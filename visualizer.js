/* ==========================================================================
   SECTION 2.4 LEARNING LABS — UI
     - Go Concurrency Lab: producer / buffered channel / worker-pool pattern
     - BST Lab: Binary Search Tree, step-by-step execution

   This is a browser-based educational simulation. It visualizes Go
   concurrency concepts but does NOT execute Go code, does NOT run a live
   backend, and no real goroutines, channels or workers are running. Every
   counter shown is produced by the JavaScript model in viz-core.js.
...
(function () {
  ...
  function applyUrlParams() {
    ...
    return { touched, auto: p.get('auto') === '1' };
  }
  ...
  function resetSim(msg) {
    ...
    $('vpLog').innerHTML = '';
    highlightCode(null);
    renderAll([{ type: 'SPAWN', msg: msg || `simulation reset — make(chan Task, ${state.sched.capacity}) with ${state.sched.workers.length} simulated worker(s)` }]);
  }
  ...
  function init() {
    ...
    if (!root || !C || !C.Scheduler || !C.BST) return;
    ...
    $('vpCfgWorkers').addEventListener('change', () => {
      ...
      syncCodeHeader();
      renderAll(evs);
      vpToast(evs.length ? 'simulated worker pool resized' : 'worker count unchanged');
    });
    ...
    ['vpCfgProducer', 'vpCfgProcessing', 'vpCfgFailRate', 'vpCfgMaxRetries'].forEach(id => {
      $(id).addEventListener('change', () => {
        ...
        vpToast('config applied to the simulation (no reset needed)');
      });
    });
